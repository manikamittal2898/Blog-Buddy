...
class TextHighlighter(object):
    ...
    def format_n_gram_text(self, text, relevant_words_array, n_gram):
        ...
        while y < len(text_tokens):
            ...
            n_gram_kw_list = []
            n_gram_word_list, splited_n_gram_kw_list = self.find_more_relevant(y, text_tokens, n_gram,
                                                                               relevant_words_array, n_gram_kw_list,
                                                                               splited_n_gram_kw_list)
            if n_gram_word_list:
                ...
                if len(n_gram_word_list[0].split(' ')) == 1:
                    ...
                    for len_kw in range(0, len(splited_one)):
                        kw_list, splited_n_gram_kw_list = self.find_more_relevant(y + len_kw, text_tokens, n_gram,
                                                                                  relevant_words_array, kw_list,
                                                                                  splited_n_gram_kw_list)
                    ...
                    if kw_list.index(min_score_word) == 0:
                        ...
                        if temporal_kw.lower() in relevant_words_array:
                            try:
                                if relevant_words_array.index(temporal_kw.lower()) > relevant_words_array.index(
                                        final_splited_text[-1].lower() + ' ' + temporal_kw.lower()) and not re.findall(
                                        self.highlight_pre, final_splited_text[-1].lower()):
    ...
    def replace_token(self, text_tokens, y, n_gram_word_list):
        ...
        new_expression = txt.replace(re.sub('[!",:.;?()]$|^[!",:.;?()]|\W["!,:.;?()]', '', txt),
